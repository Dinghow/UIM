...
def main():
    ...
    with torch.no_grad():
        ...
        for ii, sample in enumerate(val_loader):
            ...
            if args.arch == 'mgmatting':
                ...
                pred = alpha_pred_os8.detach().cpu().numpy()[0, 0, ...]
            else:
            ...
            if args.save_pic:
                ...
                image_dir = os.path.join(args.save_folder, pic_id + '_img.jpg')
                ...
                out_image = cv2.cvtColor(image * 255, cv2.COLOR_RGB2BGR)
                cv2.imwrite(image_dir, out_image)
            ...
            mse_tri_free = compute_mse(pred, alpha, trimap_ori)
            sad_tri_free = compute_sad(pred, alpha, trimap_ori)
            grad_tri_free = compute_gradient(pred, alpha, trimap_ori)
            connectivity_tri_free = compute_connectivity(pred, alpha, trimap_ori)

            if cat not in eval_result["per_categ_mse"]:
                eval_result["per_categ_mse"][cat] = [0, 0]
                eval_result["per_categ_sad"][cat] = [0, 0]
                eval_result["per_categ_grad"][cat] = [0, 0]
                eval_result["per_categ_conn"][cat] = [0, 0]
            ...
            eval_result["per_categ_mse"][cat][1] += mse_tri_free
            eval_result["per_categ_mse"][cat][0] += 1
            eval_result["per_categ_sad"][cat][1] += sad_tri_free
            eval_result["per_categ_sad"][cat][0] += 1
            eval_result["per_categ_grad"][cat][1] += grad_tri_free
            eval_result["per_categ_grad"][cat][0] += 1
            eval_result["per_categ_conn"][cat][1] += connectivity_tri_free
            eval_result["per_categ_conn"][cat][0] += 1

            # Compute some stats
        for cat in eval_result["per_categ_mse"]:
            eval_result["per_categ_mse"][cat] = float(eval_result["per_categ_mse"][cat][1] / eval_result["per_categ_mse"][cat][0])
            eval_result["per_categ_sad"][cat] = float(eval_result["per_categ_sad"][cat][1] / eval_result["per_categ_sad"][cat][0])
            eval_result["per_categ_grad"][cat] = float(eval_result["per_categ_grad"][cat][1] / eval_result["per_categ_grad"][cat][0])
            eval_result["per_categ_conn"][cat] = float(eval_result["per_categ_conn"][cat][1] / eval_result["per_categ_conn"][cat][0])
    ...
    logger.info("Per cat MSE: ")
    logger.info(sort_dict(eval_result["per_categ_mse"]))
    # pcm_list = list(eval_result["per_categ_mse"].values())
    # logger.info(sum(pcm_list) / len(pcm_list))
    logger.info("Per cat SAD: ")
    logger.info(sort_dict(eval_result["per_categ_sad"]))
    # pcs_list = list(eval_result["per_categ_sad"].values())
    # logger.info(sum(pcs_list) / len(pcs_list))
    logger.info("Per cat Grad: ")
    logger.info(sort_dict(eval_result["per_categ_grad"]))
    # pcg_list = list(eval_result["per_categ_grad"].values())
    # logger.info(sum(pcg_list) / len(pcg_list))
    logger.info("Per cat Connectivity: ")
    logger.info(sort_dict(eval_result["per_categ_conn"]))
    # pcc_list = list(eval_result["per_categ_conn"].values())
    # logger.info(sum(pcc_list) / len(pcc_list))
